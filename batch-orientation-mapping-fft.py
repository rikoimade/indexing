...
if __name__ == '__main__':
    ...
    for image in flist:
        try:
            print(image+" start")
            number_of_tiles = 16
            dp = create_diffraction_from_fft(image, number_of_tiles=16)
            dp = clean_diffraction_pattern(dp)

            saveas = image.split('.')[0]+'_stacked_fft_gekko_'+str(number_of_tiles)+'by'+str(number_of_tiles)+'_'+today+'.hspy'
            dp.save(saveas)
